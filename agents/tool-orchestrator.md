...
3. LOGIN_RUN — `/login <target>` where a pattern already exists (no work needed; tell
   the user to just call the `mcp__factory__login` tool)
...
3. Otherwise report success and tell the user to call `mcp__factory__login` again.
