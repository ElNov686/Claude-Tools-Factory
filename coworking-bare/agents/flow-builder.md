...
3. If web/desktop, open the target with a Playwright/pywinauto script, dump the
   accessible tree to a JSON file, then read that file for selectors. Do not
   paste the dump into your reasoning — keep it on disk.
...
- Pause-on-2FA via `pause_for_user`; for out-of-band approval print a clear
  message and wait for the user to confirm.
